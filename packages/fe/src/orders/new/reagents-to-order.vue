...
const emit = defineEmits(['update-reagent', 'remove-reagent', 'add-reagent', 'add-new-reagent']);
...
function toggleReagents() {
	showReagets.value = !showReagets.value;
}
...
const addNewReagent = () => {
	emit('add-new-reagent', newReagent.value);
	newReagent.value = {
		reagentName: '',
		quantityUnit: '',
		quantity: 1,
...
<template>
	<div class="orders-container" max-height="350">
		...
		<div v-for="(order, index) of props.orders" :key="index" class="order-row el-table">
			<el-form-item>
				<el-input
		...
		<div class="order-row">
			...
			<el-form-item>
				<el-input v-model="newReagent.quantity" placeholder="Enter quantity" />
			</el-form-item>
			<el-form-item>
				<el-input v-model="newReagent.amount" placeholder="Enter amount" />
			</el-form-item>
			<div>
				<el-button @click="addNewReagent"><rh-icon name="plus" /></el-button>
				<el-button v-if="showReagets" @click="toggleReagents"><rh-icon name="times" /></el-button>
				<el-button v-else @click="toggleReagents"><rh-icon name="ellipsis-v" /></el-button>
			</div>
		</div>
	</div>
	<div v-if="showReagets">
		<h2 class="el-form-item__label">Existing Reagents</h2>
		<el-table :data="props.reagents" max-height="350">
